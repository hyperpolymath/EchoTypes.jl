...
#
# Development environment for EchoTypes.jl.
#
...
# (just julia) — not a generic estate default.
#
#   nix develop      # enter the shell
#   nix flake check  # verify this file evaluates (run before committing)
{
  description = "EchoTypes.jl development environment";
    ...
    in
    {
      devShells = forAllSystems (pkgs: {
        default = pkgs.mkShell {
          packages = with pkgs; [ just julia-bin ];
        };
      });
    };
}
